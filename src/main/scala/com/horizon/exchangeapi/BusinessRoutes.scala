...
object BusinessUtils {
  def getAnyProblem(service: BService)(implicit acceptLang: Language): Option[String] = {
    // Ensure the references to the service are not null
    if (service.name==null || service.org==null || service.arch==null) return Some(ExchMsg.translate("no.service.ref.specified.for.service")(acceptLang))
    // Check they specified at least 1 service version
    if (service.serviceVersions==null || service.serviceVersions.isEmpty) return Some(ExchMsg.translate("no.version.specified.for.service2")(acceptLang))
    // Check the version syntax
    for (sv <- service.serviceVersions) {
      if (!Version(sv.version).isValid) return Some(ExchMsg.translate("version.not.valid.format", sv.version)(acceptLang))
    }
    None
  }
}
...
final case class PostPutBusinessPolicyRequest(label: String, description: Option[String], service: BService, userInput: Option[List[OneUserInputService]], properties: Option[List[OneProperty]], constraints: Option[List[String]]) {
  ...
  def getAnyProblem(implicit acceptLang: Language): Option[String] = { BusinessUtils.getAnyProblem(service) }
